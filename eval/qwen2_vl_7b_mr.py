...
bnb_config = BitsAndBytesConfig(
    load_in_4bit=True,                    # 量化到 4-bit
    bnb_4bit_compute_dtype=torch.bfloat16,  # 運算用 bfloat16
    offload_folder="/tmp/offload"         # offload 到 CPU 的暫存目錄 (只有linux/MacOS 有)
)

model = Qwen2VLForConditionalGeneration.from_pretrained(
    "Qwen/Qwen2-VL-7B-Instruct", quantization_config=bnb_config, torch_dtype=torch.bfloat16, device_map="auto"#, attn_implementation="flash_attention_2"
)
...
def annotate_and_save_video(file_path, output_file_path, position, font_size, color):
    try:
        ...
        if not cap.isOpened():
            ...
            return 
        print("file_path: ", file_path) # can be removed
    ...
    try:
        ...
        for video_info in tqdm(video_list):
            ...
            if not os.path.exists(annotated_video_path):
              print(f"Annotated video not found, skipping {video_info['video']}")
              continue
