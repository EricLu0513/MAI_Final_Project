...
def run_eval(
    ...
    Args:
        ...
        fps: original FPS used to convert frame index -> seconds.
             If <= 0, use the video's own FPS from OpenCV.
    ...
    try:
        ...
        for sample in tqdm(samples, desc="Evaluating"):
            ...
            # FPS used for converting frame index -> seconds
            fps_for_time = fps if fps > 0 else orig_fps
            ...
            # 3c) Convert frame index -> seconds for IoU (outside Qwen)
            if fps_for_time > 0:
                pred_start_sec = pred_start_frame / fps_for_time
                pred_end_sec = pred_end_frame / fps_for_time
            else:
                pred_start_sec, pred_end_sec = 0.0, 0.0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=30.0,
        help=(
            "Original FPS used to convert frame index to seconds. "
            "If <= 0, use the video's own FPS from OpenCV."
        ),
    )
    parser.add_argument(
